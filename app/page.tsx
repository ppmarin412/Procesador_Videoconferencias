...
export default function Home() {
  ...
  const handleSendBotData = () => {
    ...
    setBotState('idle');
    alert('¡Datos transferidos con éxito!');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Error al procesar la información con Gemini');
  ...
  const uploadToDrive = async (reportToUpload: MeetingReport) => {
    ...
    try {
      ...
      alert('Guardado en Google Drive completado de forma correcta.');
    } finally {
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-[#EAE3D8] border-2 border-dashed border-[#C4B49F] rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between border-b border-[#C4B49F] pb-3 mb-4">
              ...
              <span className="text-[10px] bg-[#FBF9F6] border border-[#C4B49F] text-[#65594C] px-2 py-0.5 rounded-md flex items-center gap-1 font-medium">
                <Laptop className="w-3 h-3" /> PC Local (100% Gratis)
              </span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-xs font-bold">2. Resumen detailed</p>
                ...
                <label className="flex items-start gap-3 cursor-pointer select-none">
                  ...
                  <div>
                    <span className="text-xs font-bold text-[#3D342B]">
                      5. Activar Consejos inteligentes de IA (Enriquecimiento Externo RAG)
                    </span>
                    <p className="text-[11px] text-[#7C6E5E] mt-0.5 leading-relaxed">
                      Ejecuta una búsqueda en internet en tiempo real y un análisis de riesgos cruzando keywords normativas y logísticas.
                    </p>
                  </div>
                </label>
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={5}
                  className="w-full px-4 py-3 bg-[#FBF9F6] border border-[#DCD3C5] rounded-xl text-[#4A4036] font-mono text-xs focus:outline-none focus:border-[#A38A70] resize-y"
                />
              </div>
              ...
              >
                {loading ? (
                  <span className="animate-pulse">Procesando pipeline con Gemini + RAG...</span>
                ) : (
          ...
          {/* REPORTE COMPLETO CON DONUT RESTAURADO */}
          {report && (
            <article className="bg-[#EFEAE2] rounded-2xl p-6 border border-[#DCD3C5] shadow-sm space-y-6 animate-fadeIn">
              <div className="border-b border-[#DCD3C5] pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-[#8C7E6E] mt-1.5">
                    <span><Calendar className="w-3.5 h-3.5 inline mr-1" /> {report.date}</span>
                ...
                <button
                  ...
                  className="sm:self-center text-xs font-medium uppercase text-[#FBF9F6] bg-[#A38A70] hover:bg-[#8C755E] flex items-center justify-center gap-2 px-4 py-2 rounded-xl transition-all shadow-sm"
                >
                  <Cloud className="w-4 h-4" /> {uploadingDrive === report.id ? 'Guardando...' : 'Guardar en Drive'}
                </button>
              </div>

              <section className="space-y-2">
                <h4 className="text-xs font-bold tracking-widest text-[#8C7E6E] uppercase">Resultado del Análisis</h4>
                <div className="text-sm text-[#4A4036] leading-relaxed whitespace-pre-line bg-[#FBF9F6] border border-[#EBE3D5] rounded-xl p-4 font-medium shadow-inner">
                  {report.summary}
                </div>
              </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-[#DCD3C5]">
                ...
                {/* GRÁFICO DONUT DE PARTICIPACIÓN RESTAURADO */}
                <section className="space-y-2">
              ...
              {/* BLOQUE DE CONSEJOS INTELIGENTES CON RAG */}
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {report.aiTipsContent.externalLinks.map((link, i) => (
                          <a key={i} href={link.url} target="_blank" rel="noreferrer" className="block p-3 bg-[#FBF9F6] hover:bg-[#EFEAE2] border border-[#C4B49F] rounded-lg transition-all group shadow-sm">
                            <span className="text-[9px] font-mono bg-[#E6DFD3] px-1.5 py-0.5 rounded border border-[#C4B49F] text-[#65594C] font-bold uppercase">{link.source}</span>
                            <p className="text-xs font-bold text-[#3D342B] mt-1 group-hover:text-[#65594C] underline">{link.title}</p>
                          </a>
                        ))}
                      </div>
                    </div>
          ...
          <section className="bg-[#EFEAE2] rounded-2xl p-5 border border-[#DCD3C5] shadow-sm">
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-3 max-h-[580px] overflow-y-auto pr-1">
                {history.map((h) => (
                  <div key={h.id} onClick={() => setReport(h)} className={`p-4 rounded-xl border text-left transition-all cursor-pointer bg-[#FBF9F6] ${report?.id === h.id ? 'border-[#65594C] ring-1 ring-[#65594C] shadow-sm' : 'border-[#EBE3D5] hover:border-[#C4B49F]'}`}>
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="font-semibold text-sm text-[#3D342B] truncate">{h.title}</h3>
                      <span className="text-[10px] font-mono font-medium text-[#8C7E6E] whitespace-nowrap bg-[#EFEAE2] px-2 py-0.5 border border-[#DCD3C5] rounded-md">{h.date}</span>
                    </div>
                    <p className="text-xs text-[#7C6E5E] line-clamp-2 mt-2 leading-relaxed font-medium">{h.summary}</p>
                    <div className="flex items-center justify-between gap-2 mt-3 pt-2.5 border-t border-[#EFEAE2]">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); uploadToDrive(h); }} disabled={uploadingDrive === h.id} className="text-[10px] font-medium uppercase tracking-wider text-[#7C6E5E] hover:text-[#3D342B] flex items-center gap-1 bg-[#EFEAE2] border border-[#DCD3C5] px-2.5 py-1 rounded-md transition-colors shadow-sm"><Cloud className="w-3 h-3 text-[#8C7E6E]" /> Subir a Drive</button>
                    </div>
                  </div>
                ))}
              </div>
